...
public class ParlamentarUserDao {
	...
	private ParlamentarUserDao(Context context) {
		ParlamentarUserDao.database = new LocalDatabase(context);
	}
	...
	public List<Parlamentar> getMinorRanking() {

		sqliteDatabase = database.getReadableDatabase();
		Cursor cursor = sqliteDatabase.rawQuery(
				"SELECT * FROM PARLAMENTAR order by VALOR", null);
		...
		while (cursor.moveToNext()) {
			...
			parlamentar.setIdUpdate(cursor.getInt(cursor
					.getColumnIndex("ID_ATUALIZACAO")));
			parlamentar.setMinorRankingPos(cursor.getCount() - parlamentar.getMajorRankingPos() + 1);
			listParlamentares.add(parlamentar);
